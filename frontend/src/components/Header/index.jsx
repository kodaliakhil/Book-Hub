...
function Header() {
  ...
  const renderMobileNavIconsContainer = () => (
    <HeaderContext.Consumer>
      {(value) => {
        ...
        const navIcon = isDarkTheme ? <HiSun size={25} color="#ffffff" /> : <HiMoon size={25} color="#64748b" />;
          ...
          <div className={`nav-menu-mobile ${darkThemeNavMenu}`}>
            <ul className="nav-menu-list-mobile">
              {navItems.map((eachItem) => (
                <NavItem key={eachItem.id} navItemDetails={eachItem} updateActiveNavId={updateActiveNavId} />
              ))}
  ...
  return (
    <HeaderContext.Consumer>
      {(value) => {
        const { showNavIcons, onToggleIcon, updateActiveNavId, isDarkTheme } = value;
          ...
          <nav className={`nav-header ${headerDarkThemeBg}`}>
            <div className="nav-content">
              <div className="nav-bar-mobile-logo-container">
                ...
                <button type="button" className="nav-mobile-btn" onClick={onToggleIcon}>
            ...
            {showNavIcons ? renderMobileNavIconsContainer() : null}
          </nav>
        );
      }}
    </HeaderContext.Consumer>
